...
//	************************************************************
	...
	function FastTemplate ($pathToTemplates = "templates")
	{
		global $php_errormsg;
		...
		while ( list ($key,$val) = each ($tpl_array) )
		{
			if (!(empty($key)))
			{
				...
				$template = ereg_replace("$key","$val","$template");
                           ...
                           //Fixar de gånger man behöver använda { och }
                           //$template = preg_replace("/\[(.+?)\]/is", "{\\1}\n", $template );  
...
//	************************************************************
	...
	function FastPrint ( $template = "" )
	{
		...
		{
                       $this->$template = preg_replace("/\[#(.+?)\#]/is", "{\\1}", $this->$template );
			print $this->$template;
		}
		return;
	}
